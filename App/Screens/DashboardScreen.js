...
export default class Dashboard extends React.Component {
    ...
    return(
      <TouchableHighlight underlayColor={'#f3f3f2'} onPress={() => this.selectRow(row, pushNavBarTitle)}>
        <View style={styles.rowContainer}>
          ...
          <View style={syles.rowDetailsContainer}>
  ...
  listViewOnRefresh = (page, callback, api_endpoint) => {
    if(page != 1 && this.state.topStoryIDs){
      ...
      fetch(api_endpoint).then((response) => response.json()).then((topStoryIDs) => {
  ...
  fetchStoriesUsingTopStoryIDs = (topStoryIDs, startIndex, amountToAdd, callback) => {
    var rowsData = [];
    var endIndex = (startIndex + amountToAdd) < topStoryIDs.length
      ? (startIndex + amountToAdd)
      : topStoryIDs.length;
    function iterateAndFetch() {
      if (startIndex < endIndex) {
        fetch(api.HN_ITEM_ENDPOINT + topStoryIDs[startIndex] + ".json").then((response) => response.json()).then((topStory) => {
  ...
  render(){
    return (
      <TabBar structure={[
        {
          ...
          renderContent: () => {
            return(<RefreshableListView
              rednerRow={(row) => this.renderListViewRow(row, 'ShowStory')}
          ...
          renderContent: () => {
            return( <RefreshableListView 
              renderRow={(row) => this.renderListViewRow(row, 'New Story')}
          ...
          renderContent: () => {
            return(<RefreshableListView 
              renderRow={(row) => this.renderListViewRow(row, 'Job Post')}
      ...
      ]} selectedTab={2} activeTintColor={'#ff8533'} iconSize={25}/>
    )
  }
}
